...
namespace VoodooPackages.Tool.VST
{
	public static class ExporterValidationEditor
	{
		...
		private static void ValidateFields()
		{
			...
			if (string.IsNullOrEmpty(Exporter.data.package.subCategory) && string.IsNullOrEmpty(Exporter.data.newSubCategory))
			{
				ExporterHelpBox tempExporterHelpBox = new ExporterHelpBox(exporterHelpBox)
				{
					objectType = "Missing Category",
					message    = "You need to fill the New Category field or select a proper Category"
				};
				exporterHelpBoxArea.Add(tempExporterHelpBox);
		...
		{
			Match isUnderVoodooPackagesFolder = Regex.Match(parentFolderPath, @"(VoodooPackages)", RegexOptions.IgnoreCase); ;
			
			if (isUnderVoodooPackagesFolder.Success == false)
			{
				ExporterHelpBox exporterHelpBox = new ExporterHelpBox
				{
					helpBoxType   = HelpBoxType.IncorrectPath,
					objectType    = "VoodooPackages",
					message       = "Your parent folder must be under a \"VoodooPackages\" folder.",
			...
			List<string> folderNames = new List<string>{"Scripts"};
		...
		private static void ValidateNamespaces()
		{
			...
			foreach (string file in files)
			{
				...
				int baseIndex = path.IndexOf("VoodooPackages");
				string packageName = path.ElementAt(baseIndex + 1);
				string fileContent = File.ReadAllText(file);
				string awaitedNamespace = "VoodooPackages.Tech/Tool/Asset." + packageName;
				...
				{
					Match isRightNamespace = Regex.Match(match.Value,@"((namespace)\s+(VoodooPackages)[.]\w+[.](" + packageName + "))", RegexOptions.IgnoreCase);
		...
		private static void FixAsked(ExporterHelpBox exporterHelpBox, bool silently)
		{
			...
			if (exporterHelpBox.objectType == "VoodooPackages")
			{
				MoveToVoodooPackagesFolder(filePath, folders, !silently);
			}
			else
			{
		...
		{
			int indexOfMainFolder = folders.IndexOf("VoodooPackages");
			...
			//Browse all the folders until the "VoodooPackages" one or "Assets" if there is none
		...
		private static void MoveToVoodooPackagesFolder(string filePath, List<string> folders, bool displayDialogBox)
		{
			string voodooPackagesDirectory = GetNearestVoodooPackagesDirectory(filePath);
			string newPath = Path.Combine(voodooPackagesDirectory, folders.Last());

			MoveAsset(filePath, newPath, displayDialogBox);
		}

		private static string GetNearestVoodooPackagesDirectory(string filePath)
		{
			string res = "";
			
			DirectoryInfo parentDirectory = Directory.GetParent(filePath);
			string[] folderPaths = Directory.GetDirectories(parentDirectory.FullName, "VoodooPackages", SearchOption.AllDirectories);
			...
			else
			{
				...
				{
					res = GetNearestVoodooPackagesDirectory(parentDirectory.FullName);
				}
			}
