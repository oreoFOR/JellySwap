...
{
	public static class ExporterInfoEditor
	{
        ...
        {
	        EditorGUI.BeginChangeCheck();
	        Exporter.data.categorySelected = EditorGUILayout.Popup("Category", Exporter.data.categorySelected, Exporter.data.categories);
	        if (EditorGUI.EndChangeCheck())
	        {
		        Exporter.LoadSubCategories();
	        }

	        Exporter.data.package.category = Exporter.data.categories[Exporter.data.categorySelected];
        }

        private static void ShowSubCategory()
        {
	        Exporter.data.subCategorySelected = EditorGUILayout.Popup("SubCategory", Exporter.data.subCategorySelected, Exporter.data.subCategories);

	        if (Exporter.data.subCategories[Exporter.data.subCategorySelected] == Exporter.data.newSubCategoryText)
	        {
		        EditorGUI.indentLevel++;
		        Exporter.data.newSubCategory = EditorGUILayout.TextField(Exporter.data.newSubCategoryText, Exporter.data.newSubCategory);
		        Exporter.data.package.subCategory = Exporter.data.newSubCategory;
		        EditorGUI.indentLevel--;
	        }
	        else
	        {
		        Exporter.data.package.subCategory = Exporter.data.subCategories[Exporter.data.subCategorySelected];
	        }
        }

        private static void ShowDocumentationLink()
        {
	        EditorGUILayout.BeginHorizontal();
